...
:::tip
Make one template per tier (`vip`, `mvp`, `bundle`) and let your Tebex command pick the right one. Each tier can have its own colors, sound, bossbar and Discord embed.
:::
...
:::note
Events are per-template. A "stealth" template can disable every event and still send a Discord notification, for example.
:::
...
:::tip
Use `UI_TOAST_CHALLENGE_COMPLETE` for an "achievement" feel, or `BLOCK_NOTE_BLOCK_PLING` for something less intrusive.
:::
...
:::caution
ActionBars are a single line. Long messages get truncated by the client, not wrapped.
:::
